...
# === Gửi form nhiều lần ===
try:
    so_lan = int(input("Bạn muốn gửi bao nhiêu lần? "))
except:
...
for i in range(so_lan):
    ...
    truong_weighted = [(truong, 30 if truong == target_truong else 1) for truong in truong_list]
    ...
    if response.ok:
        print(f"✅ [{i+1}/{so_lan}] Gửi thành công: {truong_duoc_chon}")
    else:
        print(f"❌ [{i+1}/{so_lan}] Gửi lỗi:", response.status_code)
        print(response.text)
